...
def execute_tool(rec: TaskRecord, name: str, args: dict[str, Any]) -> str:
    ...
    try:
        ...
        if name == "write_file":
            rel = filesystem.write_file(ws, args["path"], args.get("content") or "", globs)
            if rel not in rec.files_changed:
                rec.files_changed.append(rel)
            return f"wrote {rel}"
        if name == "patch_file":
            rel = filesystem.patch_file(ws, args["path"], args["old"], args["new"], globs)
            if rel not in rec.files_changed:
                rec.files_changed.append(rel)
        ...
        if name == "git_status":
            return git_tools.status(ws)
        if name == "git_diff":
            return git_tools.diff(ws)
        if name == "git_log":
    ...
    try:
        for i in range(max_iter):
            if rec.cancel_event.is_set():
                ...
                return rec
            rec.iteration = i + 1
            user = build_context(rec.task, extra=extra)
